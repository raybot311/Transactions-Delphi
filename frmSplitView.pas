...
type
  TSplitViewForm = class(TForm)
    ...
    DBGrid1: TDBGrid;
    dsStoredProcedureSalesCountry: TDataSource;
    BtnSearch: TButton;
    EditEndDate: TEdit;
    EditBeginDate: TEdit;
    Panel2: TPanel;
    ...
    dsSalesCategory: TDataSource;
    stProcSalesCategory: TADOStoredProc;
...
begin
  ...
  fDashPos := Pos('-',UsDate);
  fDay := Copy(UsDate,1,fDashPos -1);      //dd
  delete(UsDate,1,fDashPos);

  fYear := UsDate;                         //yyyy
  fEuDate := fYear +'-'+fMonth+'-'+fDay;   //yyyy-mm-dd
